...
export class HttpClient {
  ...
  private async request(method: 'get' | 'post' | 'patch', path: string, data?: any, headers = {}) {
    try {
      ...
      console.log('[HTTP REQUEST]', method, path, 'Headers:', builtHeaders);
  ...
  private buildHeaders() {
    console.log('🔧 buildHeaders() called');
    let headers: any = { "Content-Type": "application/json" };
    const token = StorageService.accessToken()?.raw;

    if (token) headers = Object.assign(headers, { "Authorization": `Bearer ${token}` });

    // Add fingerprint header in dev mode
    const ignoreAuth = ENV_VAR(EnvKeys.VUE_APP_IGNORE_AUTH) === 'true';
    console.log('[HTTP] IGNORE_AUTH:', ignoreAuth);

    if (ignoreAuth) {
      const cachedFingerprint = FingerprintService.getCachedFingerprint();
      console.log('[HTTP] Cached fingerprint:', cachedFingerprint);

      if (cachedFingerprint) {
        headers['X-Fingerprint'] = cachedFingerprint;
        console.log('[HTTP] Added X-Fingerprint header');
      } else {
        console.warn('[HTTP] No cached fingerprint available');
      }
    }
