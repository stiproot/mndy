---
name: mndy-mcps
description: Orientation for using the mndy MCP servers (GitHub Issues, GA4, Meta Ads, Shopify, Dapr cache) from Claude Code. Use when the user wants to connect to, start, verify, or troubleshoot the mndy MCP servers, or when you need to know which mndy MCP tool to reach for. Points to the per-server skills for tool details.
---

# Using the mndy MCP servers

The `mndy-mcp` plugin wires five MCP servers from the mndy repo into Claude Code and
documents how to use them. Use these tools directly from a shell agent, or as the tool
layer of the full mndy solution.
...
| `mndy-dapr` | `http://localhost:3006/mcp` | `dapr-cache` | cache read/write, actor state, brand reports |
...
**The analytics MCP servers — GA4, Meta Ads, and Shopify — need NO infrastructure.**
They are plain HTTP processes. Each needs only its own API credentials (in its `.env`) and
a free port. No Dapr, no MongoDB, no RabbitMQ, no `docker-compose`. `github-issues` is the
same — standalone, credentials only.

**Only `dapr` needs infrastructure**: it requires a Dapr sidecar plus the backing infra
(`make docker-compose-infra`). If you're not using the Dapr cache tools, you can ignore it
entirely.

So: to use the analytics tools, start just those servers — nothing else.
...
The `run-analytics-mcps` target builds and launches GA4, Meta Ads, and Shopify together in
one foreground command; Ctrl-C stops all three. `make run-dapr-mcp` additionally launches a
**Dapr sidecar** (needs `make docker-compose-infra` up first).
...
- Read/write the Dapr state cache, actor state, or brand reports → `dapr-cache` skill
