...
const fetchData = (url: string) => Http.get(url); // Returns Http.HttpError, not domain error
```
...
// app.ts
export const AppLive = Layer.merge(DatabaseLive, HttpLive).pipe(
...
const program = Effect.all([fetchUser, fetchPosts, fetchComments], {
...
const result =
  ...
  Effect.race(
    slowOperation,
    Effect.sleep('5 seconds').pipe(Effect.as(new TimeoutError()))
  );

// ✅ GOOD: Try multiple sources
const data = yield * Effect.race(fetchFromPrimary, fetchFromBackup);
...
const scopedFile = Effect.acquireRelease(
  Effect.log('File opened').pipe(
    Effect.as({ write: (s: string) => Effect.log(s) })
  ),
...
const validateEmail = (s: string): Either.Either<string, Email> =>
  s.includes('@') ? Either.right(s as Email) : Either.left('Invalid email');

const validateAge = (n: number): Either.Either<string, Age> =>
  n >= 18 ? Either.right(n as Age) : Either.left('Must be 18+');

// Accumulate all errors
const validation = Either.all([validateEmail(email), validateAge(age)]);
...
const parseEmail = (input: string) => Schema.decode(EmailSchema)(input);
...
const program = Effect.all(
  Array.from({ length: 1000 }, () => Effect.sync(() => counter++)),
  { concurrency: 'unbounded' }
);
  ...
  HttpRouter.get(
    ...
    Effect.flatMap(HttpRouter.params, (params) =>
      Effect.flatMap(UserService, (service) =>
        service.findUser(params.id).pipe(Effect.flatMap(HttpResponse.json))
      )
    )
  )
);
...
const processOrder = (orderId: string) =>
  ...
  }).pipe(Effect.withSpan('order.process', { attributes: { orderId } }));
...
// ❌ BAD: No metrics
const handleRequest = (req: Request) => processRequest(req);
```
...
const FailingDatabase = Layer.succeed(Database, {
  findUser: () =>
    Effect.fail(new DatabaseError({ cause: 'Connection failed' })),
});
...
const MockApiClient = Layer.succeed(ApiClient, {
  getUser: (id: string) => Effect.succeed({ id, name: 'Mock User' }),
});
...
).pipe(Stream.flatMap((users) => Stream.fromIterable(users)));
...
const fileStream = Stream.acquireRelease(openFile('data.txt'), (file) =>
  closeFile(file)
).pipe(Stream.flatMap((file) => Stream.fromEffect(readLine(file))));
...
const results = dataStream.pipe(
  Stream.grouped(100),
  Stream.mapEffect((batch) => processBatch(batch), { concurrency: 3 })
);
...
### 1. Keep Effect-TS in Service Layer, Not Components

**Pattern:** Effect-TS orchestration belongs in the service layer. UI components should work with plain Promises and async/await.

**When to Use:**

- Building React or other web framework applications
- Integrating Effect-TS with component-based UIs
- Maintaining clear architectural boundaries
- Ensuring testable, maintainable frontend code

**Key Standards:**

```typescript
// ✅ GOOD: Service layer handles Effect orchestration
...
export class UserService extends Effect.Service<UserService>()(
  "UserService",
  {
    effect: Effect.gen(function* () {
      ...
      return {
        // Service function returns Promise to UI layer
        getUser: (id: string): Promise<User> =>
          Effect.gen(function* () {
            yield* logger.log(`Fetching user ${id}`)
            const user = yield* db.findUser(id)
            yield* logger.log(`User ${id} fetched successfully`)
            return user
          }).pipe(
            Effect.retry(retryPolicy),
            Effect.timeout("5 seconds"),
            Effect.catchAll(error =>
              Effect.fail(new UserNotFoundError({ id, cause: error }))
            ),
            Effect.runPromise // Convert to Promise at service boundary
          )
      }
    }),
...
const UserProfile = ({ userId }: { userId: string }) => {
  ...
  const userService = useUserService() // Custom hook to get service

  useEffect(() => {
    // UI layer uses plain async/await
...
// ❌ BAD: Effect.gen in React components
const UserProfile = ({ userId }: { userId: string }) => {
  const [user, setUser] = useState<User | null>(null)

  useEffect(() => {
    // Don't do Effect orchestration in components!
    const program = Effect.gen(function* () {
      const db = yield* Database
      const logger = yield* Logger
      yield* logger.log(`Fetching user ${userId}`)
      const userData = yield* db.findUser(userId)
      return userData
    })

    Effect.runPromise(program).then(setUser)
    // This couples UI to Effect runtime, hard to test, breaks separation of concerns
  }, [userId])
...
- Using Effect.gen directly in React components
- Exposing Effect types in component props or state
- Mixing Effect runtime concerns with UI rendering logic
- Not establishing clear service boundaries

**Related Patterns:** return-promises-to-ui-components, handle-effect-execution-at-boundaries

---

### 2. Return Promises to UI Components

**Pattern:** Service functions should handle all Effect-TS orchestration internally and return plain Promises to the UI layer.

**When to Use:**

- All service methods that will be called from UI components
- API client wrappers used by frontend code
- Any boundary between Effect-TS and non-Effect code

**Key Standards:**

```typescript
// ✅ GOOD: Service returns Promise, handles Effect internally
...
export class ChallengeService extends Effect.Service<ChallengeService>()(
  "ChallengeService",
  {
    effect: Effect.gen(function* () {
      ...
      return {
        createChallenge: (data: CreateChallengeRequest): Promise<Challenge> =>
          Effect.gen(function* () {
            // All Effect orchestration happens here
            const currentUser = yield* auth.getCurrentUser()
            const validated = yield* Schema.decode(ChallengeSchema)(data)
            const challenge = yield* db.createChallenge({
              ...validated,
              creatorId: currentUser.id
            })
            yield* events.publish(new ChallengeCreatedEvent({ challenge }))
            return challenge
          }).pipe(
            Effect.catchTag("ValidationError", e =>
              Effect.fail(new InvalidChallengeError({ cause: e }))
            ),
            Effect.retry(retryPolicy),
            Effect.runPromise // Service boundary: Effect -> Promise
          ),

        listChallenges: (userId: string): Promise<Challenge[]> =>
          Effect.gen(function* () {
            const challenges = yield* db.findChallengesByUser(userId)
            return challenges
          }).pipe(
            Effect.runPromise
          )
    ...
    dependencies: [Database.Default, EventBus.Default, AuthService.Default]
  }
) {}
...
const ChallengeList = () => {
  ...
  useEffect(() => {
    // Clean async/await in component
    async function loadChallenges() {
      try {
        const data = await challengeService.listChallenges(userId)
        setChallenges(data)
      } catch (error) {
  ...
  return <div>{challenges.map(c => <ChallengeCard key={c.id} {...c} />)}</div>
}

// ❌ BAD: Exposing Effect types to UI layer
export class ChallengeService extends Effect.Service<ChallengeService>()(
  "ChallengeService",
  {
    effect: Effect.gen(function* () {
      ...
      return {
        // Don't return Effect to UI layer!
        createChallenge: (data: CreateChallengeRequest): Effect.Effect<Challenge, DatabaseError> =>
          db.createChallenge(data)
      }
    }),
...
// Component now has to deal with Effect types
const CreateChallenge = () => {
  const challengeService = useChallengeService()

  const handleSubmit = async (data: CreateChallengeRequest) => {
    // Component shouldn't know about Effect
    const effect = challengeService.createChallenge(data)
    await Effect.runPromise(effect) // Wrong layer for this!
  }
}
```
...
- Returning Effect types from service methods
- Forcing UI components to understand Effect.runPromise
- Not handling errors at the service boundary
- Leaking Effect-TS abstractions into UI layer

**Related Patterns:** keep-effect-in-service-layer, service-boundary-pattern

---

### 3. Handle Effect Execution at Application Boundaries

**Pattern:** Execute Effects at clear architectural boundaries: service layer, API routes, or application initialization.
...
```typescript
// ✅ GOOD: Effect execution at service boundary
// services/notificationService.ts
export class NotificationService extends Effect.Service<NotificationService>()(
  "NotificationService",
  {
    effect: Effect.gen(function* () {
      const push = yield* PushNotificationProvider
      ...
      return {
        sendNotification: (userId: string, message: string): Promise<void> =>
          Effect.gen(function* () {
            ...
            yield* db.recordNotification(userId, message)
          }).pipe(
            Effect.catchAll(error =>
              Effect.log(`Notification failed: ${error}`).pipe(
                Effect.as(undefined)
              )
            ),
            Effect.runPromise // Boundary: Effect -> Promise
          )
      }
    }),
    dependencies: [PushNotificationProvider.Default, EmailProvider.Default, Database.Default]
  }
) {}

// ✅ GOOD: Effect execution at app initialization
// app/index.tsx
async function initializeApp() {
  // Build runtime once at startup
  ...
  // Store runtime in context/DI container
  return runtime
}

// App entry point
const runtime = await initializeApp()
...
// ✅ GOOD: Custom hook to access services (via runtime)
function useService<S>(service: Context.Tag<S, S>): S {
  const runtime = useContext(RuntimeContext)
  return useMemo(
    () => Context.get(Runtime.context(runtime), service),
    [runtime, service]
  )
}

function useChallengeService() {
  return useService(ChallengeService)
}

// ❌ BAD: Effect execution scattered in components
const UserDashboard = () => {
  const [stats, setStats] = useState(null)

  useEffect(() => {
    // Don't build runtime in components!
    const program = Effect.gen(function* () {
      const db = yield* Database
      const stats = yield* db.getUserStats(userId)
      return stats
    }).pipe(
      Effect.provide(DatabaseLive)
    )

    Effect.runPromise(program).then(setStats)
    // Runtime built on every render, no service abstraction
  }, [userId])
}
```
...
- Building Effect runtimes in components
- Calling Effect.runPromise directly in UI code
- Not establishing a single runtime at app initialization
- Missing service layer abstraction

**Related Patterns:** create-reusable-runtime-from-layers, initialize-runtime-at-startup

---

### 4. Use React Hooks for UI State, Not Effect State

**Pattern:** Keep UI state in React hooks (useState, useReducer). Use Effect Ref/state only in service layer for cross-cutting concerns.

**When to Use:**

- Component-local state (form inputs, UI toggles, etc.)
- Derived UI state
- State that drives rendering
- Any state directly tied to component lifecycle

**Key Standards:**

```typescript
// ✅ GOOD: React state for UI, Effect in services
// components/ChallengeCreator.tsx
const ChallengeCreator = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // Service handles Effect orchestration
  ...
  return (
    <form onSubmit={handleSubmit}>
      <input
        value={formData.trickId}
        onChange={e => setFormData({ ...formData, trickId: e.target.value })}
      />
...
// ✅ GOOD: Effect Ref in service layer for caching
// services/trickService.ts
export class TrickService extends Effect.Service<TrickService>()(
  "TrickService",
  {
    effect: Effect.gen(function* () {
      const db = yield* Database
      const cache = yield* Ref.make<Map<string, Trick>>(new Map())

      return {
        getTrick: (id: string): Promise<Trick> =>
          Effect.gen(function* () {
            ...
            const trick = yield* db.findTrick(id)
            yield* Ref.update(cache, map => new Map(map).set(id, trick))
            return trick
...
// ❌ BAD: Trying to use Effect Ref in React component
const ChallengeCreator = () => {
  // Don't use Effect state primitives in components!
  const [formRef, setFormRef] = useState<Ref.Ref<FormData> | null>(null)

  useEffect(() => {
    Effect.runPromise(Ref.make(initialFormData)).then(setFormRef)
    // This is wrong! Use React useState instead
  }, [])

  const handleChange = (field: string, value: string) => {
    if (formRef) {
      Effect.runPromise(
        Ref.update(formRef, data => ({ ...data, [field]: value }))
      )
      // Unnecessarily complex, breaks React rendering model
    }
  }
}
```
...
- Using Effect Ref/state in React components
- Mixing Effect state management with React rendering
- Not understanding that React hooks are the UI state layer
- Over-engineering simple UI state with Effect primitives

**Related Patterns:** manage-shared-state-with-ref, react-state-vs-effect-state

---

### 5. Initialize Effect Runtime Once at App Startup

**Pattern:** Create a single Effect runtime at application initialization and provide it via React Context.

**When to Use:**

- React application startup
- Next.js app initialization
- Any single-page application setup
- Testing setup for component tests

**Key Standards:**

```typescript
// ✅ GOOD: Single runtime initialized at startup
// app/runtime.ts
import { Layer, Effect, Runtime } from "effect"
import { DatabaseLive } from "./services/database"
import { AuthServiceLive } from "./services/auth"
import { ChallengeServiceLive } from "./services/challenge"
...
export async function createAppRuntime() {
  return await Effect.runPromise(Layer.toRuntime(AppLive))
}

// app/context/RuntimeContext.tsx
import { createContext, useContext, ReactNode } from "react"
import type { Runtime } from "effect"

const RuntimeContext = createContext<Runtime.Runtime<AppServices> | null>(null)
...
// app/hooks/useService.ts
import { Context } from "effect"
import { useRuntime } from "../context/RuntimeContext"
import { useMemo } from "react"

export function useService<S>(service: Context.Tag<S, S>): S {
...
const ChallengeList = () => {
  const challengeService = useService(ChallengeService)
...
// ❌ BAD: Creating runtime in every component
const ChallengeList = () => {
  const [challenges, setChallenges] = useState([])

  useEffect(() => {
    async function load() {
      // Don't create runtime per component!
      const runtime = await Effect.runPromise(Layer.toRuntime(AppLive))
      const service = Context.get(Runtime.context(runtime), ChallengeService)
...
function useChallenges() {
  ...
  useEffect(() => {
    const program = Effect.gen(function* () {
      const db = yield* Database
      return yield* db.findChallenges()
    }).pipe(
      Effect.provide(DatabaseLive) // Rebuilding layer every time
    )

    Effect.runPromise(program).then(setChallenges)
  }, [])
...
- Creating multiple Effect runtimes throughout the app
- Building layers in every component or hook
- Not using React Context for runtime distribution
- Rebuilding services on every render

**Related Patterns:** create-reusable-runtime-from-layers, dependency-injection-with-context

---

### 6. Test Components with Mock Service Implementations

**Pattern:** Test React components by providing mock service implementations via test runtime.

**When to Use:**

- Unit testing React components
- Integration testing UI flows
- Testing error states and edge cases
- Isolating component logic from real services

**Key Standards:**

```typescript
// ✅ GOOD: Mock services for component testing
// __tests__/ChallengeCreator.test.tsx
...
describe("ChallengeCreator", () => {
  it("creates a challenge successfully", async () => {
    // Create mock service
    const mockChallenge = { id: "1", trickId: "kickflip", targetUserId: "user123" }

    const MockChallengeService = Layer.succeed(
      ChallengeService,
      ChallengeService.of({
        createChallenge: async (data) => mockChallenge,
    ...
    // Create test runtime
    const runtime = await Effect.runPromise(
      Layer.toRuntime(MockChallengeService)
    )

    // Render with test runtime
    render(
      <RuntimeProvider runtime={runtime}>
        <ChallengeCreator />
      </RuntimeProvider>
    ...
    fireEvent.change(screen.getByLabelText("Trick"), {
      target: { value: "kickflip" }
    })
  ...
  it("handles creation errors", async () => {
    ...
    const MockFailingService = Layer.succeed(
      ChallengeService,
      ChallengeService.of({
    ...
    const runtime = await Effect.runPromise(
      Layer.toRuntime(MockFailingService)
    )
...
describe("ChallengeCreator", () => {
  it("creates a challenge", async () => {
    // Using real runtime with real services
    const runtime = await Effect.runPromise(Layer.toRuntime(AppLive))
    ...
    // This makes real database calls, network requests, etc.
    // Slow, flaky, has side effects
  })
})
```
...
- Testing with real service implementations
- Not providing runtime in tests
- Forgetting to mock external dependencies
- Making actual API calls in component tests

**Related Patterns:** mock-dependencies-in-tests, testing-with-layers
