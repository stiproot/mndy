...
import type { GetOrdersInput, OrdersResult } from "shopify-core";
 ...
 */
export function registerGetOrdersTool(server: McpServer, run: ToolRunner): void {
  server.registerTool(
    "shopify_get_orders",
    {
    ...
    (args) => {
      ...
      return run(getOrdersEffect(input));
    }
  );
}
