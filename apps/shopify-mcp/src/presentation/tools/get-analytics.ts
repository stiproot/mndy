...
import type { GetAnalyticsInput, AnalyticsSummary } from "shopify-core";
 ...
 */
export function registerGetAnalyticsTool(server: McpServer, run: ToolRunner): void {
  server.registerTool(
    ...
    (args) => {
      ...
      return run(getAnalyticsEffect(input));
    }
  );
}
