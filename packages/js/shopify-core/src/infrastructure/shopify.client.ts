import { Config, Duration, Effect, Ref, Schedule } from "effect";
...
/**
 * Shopify credentials. Auth is an OAuth client-credentials grant bound to ONE store,
 * which is why the shopify_* tools take no store parameter — see
...
export class ShopifyClient extends Effect.Service<ShopifyClient>()("ShopifyClient", {
  effect: Effect.gen(function* () {
    ...
    return {
      /** Whether credentials are configured; false means every request will fail. */
      hasCredentials: clientId !== "" && clientSecret !== "",

      /** The single store this process is bound to. */
      storeUrl,
