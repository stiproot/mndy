/**
 * Test setup utilities for ga4-mcp integration tests
...
export const TestConfig = Effect.gen(function* () {
  const mcpUrl = yield* Config.string("GA4_MCP_URL").pipe(
    Config.withDefault("http://localhost:3003")
  );
  const propertyId = yield* Config.string("GA4_TEST_PROPERTY_ID").pipe(
    Config.withDefault("")
  );

  return { mcpUrl, propertyId };
});
