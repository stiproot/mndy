/**
 * Test setup utilities for meta-ads-mcp integration tests
...
export const TestConfig = Effect.gen(function* () {
  const mcpUrl = yield* Config.string("META_MCP_URL").pipe(
    Config.withDefault("http://localhost:3004")
  );
  const adAccountId = yield* Config.string("META_TEST_AD_ACCOUNT_ID").pipe(
    Config.withDefault("")
  );

  return { mcpUrl, adAccountId };
});
